...
另一个新手几乎必撞的坑:**回车在目录上不会进入目录**。回车是 `open`,走打开规则,
而目录命中的第一条规则默认是 `edit`(`${EDITOR:-vi}`)——于是 vim 被叫起来打开一个目录,
你看到的就是 netrw directory listing(vim 内置的目录浏览器)。进目录用 `l`。
如果希望回车"目录就进入、文件就打开",装官方 smart-enter 插件
(`ya pkg install yazi-rs/plugins:smart-enter`),再把回车的键位指过去:
